...
export class WriteSync {
  ...
  static async create(
    ...
    if (debug) console.time(`[WriteSync] create ${modelName}`);
    ...
    cacheManager.cacheDocument(modelName, leanDoc).catch(err => {
      if (debug) console.error('[WriteSync] Cache sync failed:', err);
    });
    
    cacheManager.addToCollection(modelName, doc._id).catch(err => {
      if (debug) console.error('[WriteSync] Collection sync failed:', err);
    });

    if (debug) console.timeEnd(`[WriteSync] create ${modelName}`);
    return doc;
  }
  ...
  static async updateOne(
    ...
    if (debug) console.time(`[WriteSync] updateOne ${modelName}`);
    ...
    if (debug) console.timeEnd(`[WriteSync] updateOne ${modelName}`);
    return result;
  }
  ...
  static async updateMany(
    ...
    if (debug) console.time(`[WriteSync] updateMany ${modelName}`);
    ...
    if (debug) console.timeEnd(`[WriteSync] updateMany ${modelName}`);
    return result;
  }
  ...
  static async deleteOne(
    ...
    if (debug) console.time(`[WriteSync] deleteOne ${modelName}`);
    ...
    if (docId) {
      cacheManager.deleteDocument(modelName, docId).catch(err => {
        if (debug) console.error('[WriteSync] Cache delete failed:', err);
      });
      
      cacheManager.removeFromCollection(modelName, docId).catch(err => {
        if (debug) console.error('[WriteSync] Collection remove failed:', err);
      });
    }

    if (debug) console.timeEnd(`[WriteSync] deleteOne ${modelName}`);
    return result;
  }
  ...
  static async deleteMany(
    ...
    if (debug) console.time(`[WriteSync] deleteMany ${modelName}`);
    ...
    if (debug) console.timeEnd(`[WriteSync] deleteMany ${modelName}`);
    return result;
  }
  ...
  static async replaceOne(
    ...
    if (debug) console.log(`[WriteSync] Replacing ${modelName}`);
    ...
    if (doc) {
      ...
      if (debug) console.log(`[WriteSync] ✓ Replaced ${modelName}:${doc._id}`);
    }
  ...
  static async bulkWrite(
    ...
    if (debug) console.log(`[WriteSync] Bulk writing ${modelName}:`, operations.length);
    ...
    if (debug) console.log(`[WriteSync] ✓ Bulk write completed`);
